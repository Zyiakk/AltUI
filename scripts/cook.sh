...
if [ ! -f "$COOKED/Project/Classes/TKA_PlayerCameraManager.uasset" ]; then echo "COOK: hook missing - fallback -cookdir Project/Classes";
  "$(dirname "$0")/ue.sh" "$UPROJECT" -run=cook -targetplatform=LinuxNoEditor -iterate -unattended -nullrhi -nosplash -stdout -cookdir="$KIT/Content/Project/Classes" 2>&1 | grep -E "LogCook|Error" | tail -20; fi
